...
wb =        load_workbook("Parameters_ALD_LHAR.xlsx", data_only=True)
# Selecting the active worksheet
ws =        wb["LHAR_Parameters"]
# Reading values from specific cells
Runcode =   ws["B1"].value
H =         ws["B3"].value      # Cavity height (m)
W =         ws["B4"].value      # Cavity width (m)
AR =        ws["B5"].value      # Aspect ratio (-)
c =         ws["B6"].value      # Sticking coefficient (-)
Pd =        ws["B7"].value      # Desorption probability (1/s)
q0 =        ws["B8"].value      # Adsorption capacity (#/m2) 
...
Cdes =      q0*Pd                    # Constant term for fdes = Cdes*theta
...
def system(t, y):
    ...
    # Boundary conditions
    d2PAdx2[0] =    0         
    d2PAdx2[-1] =   (PA[-2] - PA[-1]) / dx**2  
    dPAdt =         D*d2PAdx2 - 4*R*T*(((1-Theta)*Cads*PA)-(Theta*Cdes))/(h*NA)
    dThetadt =      (((1-Theta)*Cads*PA)-(Theta*Cdes))/q0
...
PA_sol =        sol.y[:Nx, :]   # Partial pressure of A along the space domain and the time domain  
Theta_sol =     sol.y[Nx:, :]   # Surface coverage of A along the space domain and the time domain  

# Extracting the data corresponding to the pulse time
PA_final =      PA_sol[:, -1]  # pA(x, t_final)
Theta_final =   Theta_sol[:, -1]  # theta(x, t_final)

# ========== CALCULATIONS FROM FINAL PROFILE ===========

# ----- Penetration depth and slope at different theta values -----
...
for theta_target in theta_targets:
    ...
    # Slope (theta vs dimensionless distance) around theta value 
    slope =         abs(H * (Theta2 - Theta1) / (x2 - x1))
    
    # Difference in theta values (%)
...
# Growth along the channel atoms/m2
growth =    q0 * Theta_final
...
param_data = {
    "Parameter": ["Running Code","Script's name & version:", "Cavity height, H (m)", "Cavity width, W (m)", "Aspect ratio, AR=L/H (-)", "Sticking coefficient, c (-)", "Desorption probability, Pd (1/s)", "Adsorption capacity, q (m-2)","Pulse time (s)", "T (K)", "pA0 (Pa)", "pI (Pa)", "MA (kg/mol)", "dA (m)", "MI (kg/mol)","dI (m)", "Number of spatial grid points", "Number of time grid points", "Additional θ value for reporting penetration depth and slope (-) "],
    "Value": [Runcode, ScriptN, H, W, AR, c, Pd, q0, t_pulse, T, pA0, pI, MA, dA, MI, dI, Nx, Nt, theta_add]
}
param_df = pd.DataFrame(param_data)

calcvalues_data = {
   "Calculated Values": ["Cavity length, L (m)", "Hydraulic diameter, h (m)","Average speed of molecules A, vA (m/s)", "Knudsen diffusion coefficient of A, DKn (m2/s)", "Gas-phase diffusion coefficient of reactant A, DA (m2/s)", "Effective diffusion coefficient, Deff (m2/s)", "Knudsen number (-)", "Thiele modulus (-)", "Diff. between the two data points (y-axis) around theta=0.5 (%)", "Penetration depth at 50% coverage (m)", "Penetration depth (x/H) at 50% coverage (-)", "Slope at 50% coverage, from theta vs (x/H) data, Absolute value (-)", "Back extracted 'c', only for Kn>>1 (Arts et al. 2019) (-)", "Exposure (Pa-s)", "Theta integral, ∫θ(x)dx (µm)", "Growth integral, ∫Growth(x)dx (atoms*µm/nm^2)"],
...
data_df = pd.DataFrame({
    ...
    "pA (Pa)": PA_final,
    "theta (-)": Theta_final,
    "Growth ≡ q0*theta (atoms/nm2) ": growth/1e18,
})
...
# --- Exporting to Excel ---
run_label = Runcode
filename1 = f"{run_label}_results.xlsx"
...
# --- Final messages ---
print(f"Excel with results saved as: {run_label}_results.xlsx")
