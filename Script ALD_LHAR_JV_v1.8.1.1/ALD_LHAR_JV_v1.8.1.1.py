...
calcvalues_data = {
   "Calculated Values": ["Cavity length, L (m)", "Hydraulic diameter, h (m)","Average speed of molecules A, vA (m/s)", "Knudsen diffusion coefficient of A, DKn (m2/s)", "Gas-phase diffusion coefficient of reactant A, DA (m2/s)", "Effective diffusion coefficient, Deff (m2/s)", "Knudsen number (-)", "Thiele modulus (-)", "Diff. between the two data points (y-axis) around theta=0.5 (%)", "Penetration depth at θ=0.5 (m)", "Penetration depth (x/H) at θ=0.5 (-)", "Slope at θ=0.5, from theta vs (x/H) data, absolute value (-)", "Back extracted 'c', only for Kn>>1 (Arts et al. 2019) (-)", "Exposure (Pa-s)", "Theta integral, ∫θ(x)dx (µm)", "Growth integral, ∫Growth(x)dx (atoms*µm/nm^2)"],
